...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            // 撤销审批
            removeApproval: function(id) {
                $.modal.confirm("确定撤销审批吗？", function() {
                    var url = $.common.isEmpty(id) ? $.table._option.removeUrl : $.table._option.removeUrl.replace("{id}", id);
                    var data = { "ids": id };
                    $.operate.submit(url, "post", "json", data);
                });
            ...
            // 审批
            approval: function(id, name) {
                // console.log(name)
                // $.modal.confirm("确定 "+name+" 通过审批吗？",function() {
                    var url = $.common.isEmpty(id) ? $.table._option.approvalUrl : $.table._option.approvalUrl.replace("{id}", id);
                    // var data = { "ids": id };
                    // $.operate.submit(url, "post", "json", data);
            ...
            editDetail: function(id) {
                var _url = $.common.isEmpty(id) ? $.table._option.updateUrl : $.table._option.updateUrl.replace("{id}", id);
                layer.open({
                    type: 2,
                    area: ['800px', ($(window).height() - 50) + 'px'],
                    fix: false,
